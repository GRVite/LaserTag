...
### mio
wake_base = nts.IntervalSet(start = wake_ep.loc[0,'start'], end=opto_ep.loc[0,'start']-1000000)
...
plt.polar(tuning_curves_base[neuron], color ='tan')
plt.fill(tuning_curves_base[neuron],"tan", alpha = 0.15) 
plt.polar(tuning_curves_stim[neuron], color = 'steelblue')
plt.fill(tuning_curves_stim[neuron],"lightcyan") 
# plt.title("Neuron_" + str(neuron))
plt.legend(lista)
plt.savefig(dir2save_plots + '/tun_baseVSstim_' + str(n) + '.pdf', bbox_inches = 'tight') 


plt.figure(figsize=[20,80])
    ax = plt.subplot(1,1,1, projection='polar')
    ax.plot(tuning_curves_base[n], color ='black')
    # ax.polar(tuning_curves_base[neuron], color ='tan')
...
for i in range(len(opto_ep)):
    ...
    spikes_list.append(t)
np.save("{}{}".format(data_directory, "spikes_all"), spikes_list)

fig, (ax1,ax2) = plt.subplots(2, 1, sharex=True)
lineSize=0.5
# left, bottom, width, height = (0, 0, 5000, len(spikes_list))
# rect = plt.Rectangle((left, bottom), width, height, facecolor="limegreen", alpha=0.1)
# ax1.add_patch(rect)
ax1.eventplot(spikes_list, linelengths = 30, color='black')
...
np.save(data_directory_load, spikes_list_h)
##
...
# ax1.set_xticks( [*range(-int(timepre/1000), int(timepost/1000+5), 5)] )
ax1.set_xticklabels( [*range(-int(timepre/1000), int(timepost/1000+txticks), txticks)] )
ax2.set_xlabel('Time (us)')
ax2.set_ylabel('spikes')
plt.show()
plt.savefig('{}{}{}{}'.format(data_directory, '/plots', '/spikes_eph_' + neuron, '.pdf'))
#ax2.hist(spikes[neuron].restrict(interval).as_units('s').index, bins = 10)
